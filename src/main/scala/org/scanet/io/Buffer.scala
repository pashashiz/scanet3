...
class Buffer[@sp A: TensorType](val original: JavaBuffer) extends Comparable[Buffer[A]] {
  ...
  def toArray: Array[A] = {
    ...
    val array = Array.ofDim[A](limit - position)(TensorType[A].classTag)
    while (hasRemaining) {
...
object Buffer {
  ...
  def apply[@sp A: TensorType](original: JavaBuffer): Buffer[A] = new Buffer[A](original)
  ...
  def wrap[@sp A: TensorType](array: Array[A]): Buffer[A] = wrap(array, 0, array.length)

  def tabulate[@sp A: TensorType](capacity: Int)(f: Int => A): Buffer[A] = {
