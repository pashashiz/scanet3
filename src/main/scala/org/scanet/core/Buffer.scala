...
import org.scanet.core.TfType._
import org.scanet.core.TfType.syntax._

import scala.{specialized => sp}

class Buffer[@sp A: TfType](val original: JavaBuffer) extends Comparable[Buffer[A]] {
  ...
  def slice: Buffer[A] = TfType[A].tag match {
    ...
    case _ => error(s"Buffer[${TfType[A].show}] is not supported")
  }

  def duplicate: Buffer[A] = TfType[A].tag match {
  ...
  def asReadOnlyBuffer: Buffer[A] = TfType[A].tag match {
  ...
  def put(f: A): Buffer[A] = TfType[A].tag match {
  ...
  def get(index: Int): A = TfType[A].tag match {
  ...
  def put(index: Int, f: A): Buffer[A] = TfType[A].tag match {
  ...
  def put(src: Buffer[A]): Buffer[A] = TfType[A].tag match {
  ...
  def put(src: Array[A], offset: Int, length: Int): Buffer[A] = TfType[A].tag match {
  ...
  def compact: Buffer[A] = TfType[A].tag match {
  ...
  def get(dst: Array[A], offset: Int, length: Int): Buffer[A] = TfType[A].tag match {
  ...
  def toArray: Array[A] = {
    ...
    val array = Array.ofDim[A](limit - position)(TfType[A].classTag)
    while (hasRemaining) {
  ...
  override def toString: String = s"Buffer[${TfType[A].show}](capacity=$capacity, position=$position, limit=$limit, direct=$isDirect)" + show()
  ...
  override def compareTo(that: Buffer[A]): Int = TfType[A].tag match {
...
object Buffer {
  ...
  def apply[@sp A: TfType](original: JavaBuffer): Buffer[A] = new Buffer[A](original)

  def from[@sp A: TfType](buffer: ByteBuffer): Buffer[A] = {
    TfType[A].tag match {
  ...
  def allocate[@sp A: TfType](capacity: Int): Buffer[A] = {
    TfType[A].tag match {
  ...
  def wrap[@sp A: TfType](array: Array[A], offset: Int, length: Int): Buffer[A] = {
    TfType[A].tag match {
  ...
  def wrap[@sp A: TfType](array: Array[A]): Buffer[A] = wrap(array, 0, array.length)

  def tabulate[@sp A: TfType](capacity: Int)(f: Int => A): Buffer[A] = {
