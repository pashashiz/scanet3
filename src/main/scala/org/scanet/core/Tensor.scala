...
import org.scanet.native.{Disposable, RawTensors}
...
class Tensor[A: TensorType](private val ref: TensorRef[A], val view: View) {
  ...
  def apply[S1: CanBuildSliceFrom, S2: CanBuildSliceFrom, S3: CanBuildSliceFrom](
      s1: S1,
      s2: S2,
      s3: S3): Tensor[A] = slice(s1, s2, s3)
  def apply[
      S1: CanBuildSliceFrom,
      S2: CanBuildSliceFrom,
      S3: CanBuildSliceFrom,
      S4: CanBuildSliceFrom](s1: S1, s2: S2, s3: S3, s4: S4): Tensor[A] = slice(s1, s2, s3, s4)

  def slice[S1: CanBuildSliceFrom](s1: S1): Tensor[A] = slice(Projection(s1))
  def slice[S1: CanBuildSliceFrom, S2: CanBuildSliceFrom](s1: S1, s2: S2): Tensor[A] = slice(
    Projection(s1, s2))
  def slice[S1: CanBuildSliceFrom, S2: CanBuildSliceFrom, S3: CanBuildSliceFrom](
      s1: S1,
      s2: S2,
      s3: S3): Tensor[A] = slice(Projection(s1, s2, s3))
  def slice[
      S1: CanBuildSliceFrom,
      S2: CanBuildSliceFrom,
      S3: CanBuildSliceFrom,
      S4: CanBuildSliceFrom](s1: S1, s2: S2, s3: S3, s4: S4): Tensor[A] = slice(
    Projection(s1, s2, s3, s4))
...
object Tensor {
  ...
  def vector[A: TensorType](elements: A*): Tensor[A] = vector(
    elements.toArray(TensorType[A].classTag))
  ...
  def matrix[A: TensorType](rows: Array[A]*): Tensor[A] = {
    ...
    val data = rows
      .foldLeft(new ArrayBuffer[A](rowSizes.sum))((buffer, row) => buffer ++= row)
      .toArray(TensorType[A].classTag)
  ...
  def tabulate[A: TensorType](shape: Shape)(f: List[Int] => A): Tensor[A] = {
    ...
    val array =
      Array.tabulate[A](shape.power)(index => f(shape.indexOf(index)))(TensorType[A].classTag)
    Tensor(array, shape)
  }
...
sealed trait TensorRef[A] {
  def native: RawTensor
  def view: TensorRef[A]
}

class NativeRef[A: TensorType](val native: RawTensor)
    extends Disposable(() => native.close())
    with TensorRef[A] {
...
class ParentRef[A: TensorType](private val origin: TensorRef[A]) extends TensorRef[A] {
  val native: RawTensor = origin.native
