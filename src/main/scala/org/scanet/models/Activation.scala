...
object Activation {
  ...
  case object Tanh extends Activation {
    override def build[A: Numeric : Floating : TensorType](in: Output[A]): Output[A] = in.tanh
  }
  ...
  case class ReLU(threshold: Float = 0f) extends Activation {
    override def build[A: Numeric : Floating : TensorType](in: Output[A]) = max(threshold.const.cast[A], in)
  }
}