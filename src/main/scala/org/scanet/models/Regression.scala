...
object Regression {
  ...
  def linear: Model[Float, Float, Float] = {
    Model[Float, Float, Float]((batchShape, weightsShape) => {
      // batch: (n, m - 1), weights: (m)
      val batch = placeholder[Float](batchShape)
      val weights = placeholder[Float](weightsShape)
      ...
      val result = (0.5f / rows).const :* (bx * wt - y).pow(2).sum
      (batch, weights, result)
    })
  }
}
